...
class Downsample3D(nn.Module):
    def __init__(self, in_channels: int, with_conv: bool) -> None:
        ...
        self.with_conv = with_conv
        if with_conv:
            self.conv = nn.Conv3d(in_channels, in_channels, 3, 2, 1)
...
class VoxelFeatureVAE(nn.Module):
    ...
    def forward_encoder(self, x: torch.Tensor) -> Tuple[torch.Tensor, list]:
        ...
        x = rearrange(x, "b x y z c -> b c z x y")
    ...
    def forward_decoder(self, z: torch.Tensor, shapes: list, input_shape: Tuple[int, int, int, int, int]) -> torch.Tensor:
        ...
        expected_shape = (input_shape[3], input_shape[1], input_shape[2])
        if tuple(decoded.shape[-3:]) != expected_shape:
            decoded = F.interpolate(decoded, size=expected_shape, mode="nearest")
        decoded = rearrange(decoded, "b c z x y -> b x y z c")
    ...
    def loss(
        ...
        if mask is not None:
            ...
            occ_mask = mask.unsqueeze(-1).expand_as(sq_error)
            empty_mask = ~occ_mask
            occ_loss = sq_error[occ_mask].mean() if occ_mask.any() else sq_error.new_tensor(0.0)
            empty_loss = sq_error[empty_mask].mean() if empty_mask.any() else sq_error.new_tensor(0.0)
            occ_contrib = (sq_error * mask.float().unsqueeze(-1)).mean()
            empty_contrib = (sq_error * (1.0 - mask.float()).unsqueeze(-1) * empty_weight).mean()
            occ_ratio = mask.float().mean()
        else:
            weight = 1.0
            occ_loss = (sq_error * weight).mean()
            empty_loss = sq_error.new_tensor(0.0)
            occ_contrib = occ_loss
            empty_contrib = sq_error.new_tensor(0.0)
            occ_ratio = sq_error.new_tensor(1.0)
        recon_loss = (sq_error * weight).mean()
        kl_loss = -0.5 * torch.mean(1 + z_logvar - z_mu.pow(2) - z_logvar.exp())
        total = recon_loss + float(self.kl_weight) * kl_loss
        if return_details:
            details = {
                "recon_occ": occ_loss,
                "recon_empty": empty_loss,
                "recon_occ_contrib": occ_contrib,
                "recon_empty_contrib": empty_contrib,
                "occ_ratio": occ_ratio,
            }
            return total, recon_loss, kl_loss, details
